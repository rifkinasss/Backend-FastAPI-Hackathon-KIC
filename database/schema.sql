...
-- Uses a CTE to capture the generated device UUID for use
-- in subsequent inserts.
...
BEGIN
    ...
    -- ----- Device -----
    INSERT INTO devices (device_code, device_name, location, latitude, longitude, firmware_ver, description)
    VALUES (
        'ESP32-MINE-001',
        'SIMOSI Node Alpha',
        'Area Tambang Utama - Pit 1',
        -1.8542000,
        116.2156000,
        '1.0.0',
        'Primary monitoring node deployed at the main mining pit. Monitors dust, gas, and heavy equipment emissions.'
    )
    ...
    VALUES ('DHT22', 'DHT22 Temperature & Humidity Sensor', 'Aosong Electronics', 'Digital temperature and humidity sensor with ±0.5°C accuracy and ±2% RH.', 'digital')
    ...
    INSERT INTO sensor_definitions (sensor_code, sensor_name, manufacturer, description, interface_type)
    VALUES ('MQ4', 'MQ-4 Methane Gas Sensor', 'Winsen Electronics', 'Semiconductor gas sensor for detecting methane (CH4). Range: 300–10000 ppm.', 'analog')
    ON CONFLICT (sensor_code) DO UPDATE SET
        sensor_name    = EXCLUDED.sensor_name,
        manufacturer   = EXCLUDED.manufacturer,
        description    = EXCLUDED.description,
    ...
    INSERT INTO sensor_definitions (sensor_code, sensor_name, manufacturer, description, interface_type)
    VALUES ('MQ7', 'MQ-7 Carbon Monoxide Sensor', 'Winsen Electronics', 'Semiconductor gas sensor for detecting carbon monoxide (CO). Range: 20–2000 ppm.', 'analog')
    ON CONFLICT (sensor_code) DO UPDATE SET
        sensor_name    = EXCLUDED.sensor_name,
        manufacturer   = EXCLUDED.manufacturer,
        description    = EXCLUDED.description,
    ...
    INSERT INTO sensor_definitions (sensor_code, sensor_name, manufacturer, description, interface_type)
    VALUES ('MQ135', 'MQ-135 Air Quality Sensor', 'Winsen Electronics', 'Semiconductor gas sensor for detecting CO2, NH3, benzene, and other gases. Used for CO2 estimation.', 'analog')
    ON CONFLICT (sensor_code) DO UPDATE SET
        sensor_name    = EXCLUDED.sensor_name,
        manufacturer   = EXCLUDED.manufacturer,
        description    = EXCLUDED.description,
    ...
    INSERT INTO sensor_definitions (sensor_code, sensor_name, manufacturer, description, interface_type)
    VALUES ('DS3231', 'DS3231 Real-Time Clock', 'Maxim Integrated', 'High-precision I2C real-time clock with ±2 ppm accuracy and battery backup.', 'i2c')
    ON CONFLICT (sensor_code) DO UPDATE SET
        sensor_name    = EXCLUDED.sensor_name,
        manufacturer   = EXCLUDED.manufacturer,
        description    = EXCLUDED.description,
    ...
    VALUES (v_dht22_id, 'temperature', 'Temperature', '°C', -40.0000, 80.0000, 2, 'Ambient temperature measurement from DHT22.')
    ON CONFLICT (sensor_def_id, parameter_code) DO NOTHING;

    INSERT INTO sensor_parameters (sensor_def_id, parameter_code, parameter_name, unit, min_value, max_value, precision_dp, description)
    VALUES (v_dht22_id, 'humidity', 'Relative Humidity', '%', 0.0000, 100.0000, 2, 'Relative humidity measurement from DHT22.')
    ON CONFLICT (sensor_def_id, parameter_code) DO NOTHING;

    -- MQ4 parameters
    INSERT INTO sensor_parameters (sensor_def_id, parameter_code, parameter_name, unit, min_value, max_value, precision_dp, description)
    VALUES (v_mq4_id, 'ch4', 'Methane (CH₄)', 'ppm', 300.0000, 10000.0000, 4, 'Methane gas concentration from MQ4 sensor.')
    ON CONFLICT (sensor_def_id, parameter_code) DO NOTHING;

    -- MQ7 parameters
    INSERT INTO sensor_parameters (sensor_def_id, parameter_code, parameter_name, unit, min_value, max_value, precision_dp, description)
    VALUES (v_mq7_id, 'co', 'Carbon Monoxide (CO)', 'ppm', 20.0000, 2000.0000, 4, 'Carbon monoxide gas concentration from MQ7 sensor.')
    ON CONFLICT (sensor_def_id, parameter_code) DO NOTHING;

    -- MQ135 parameters
    INSERT INTO sensor_parameters (sensor_def_id, parameter_code, parameter_name, unit, min_value, max_value, precision_dp, description)
    VALUES (v_mq135_id, 'co2_estimated', 'Estimated CO₂', 'ppm', 400.0000, 5000.0000, 4, 'Estimated CO2 concentration derived from MQ135 analog reading.')
    ON CONFLICT (sensor_def_id, parameter_code) DO NOTHING;

    -- DS3231 parameters
    INSERT INTO sensor_parameters (sensor_def_id, parameter_code, parameter_name, unit, min_value, max_value, precision_dp, description)
    VALUES (v_rtc_id, 'rtc_datetime', 'RTC Date/Time', 'datetime', NULL, NULL, 0, 'Real-time clock timestamp from DS3231.')
    ON CONFLICT (sensor_def_id, parameter_code) DO NOTHING;

    -- Future sensor parameters (pre-registered for when sensors are installed)
    -- MQ136 → H2S (will be added as sensor_definition when physically deployed)
    -- PMS5003 → PM2.5, PM10, PM1.0
    -- MH-Z19B → CO2 (precise)

    -- ----- Device-Sensor Mappings (GPIO / I2C) -----
    INSERT INTO device_sensors (device_id, sensor_def_id, gpio_pin, i2c_address, install_date, notes)
    VALUES (v_device_id, v_dht22_id, 'GPIO4', NULL, CURRENT_DATE, 'DHT22 on digital pin GPIO4.')
    ON CONFLICT (device_id, sensor_def_id) DO UPDATE SET
        gpio_pin     = EXCLUDED.gpio_pin,
        notes        = EXCLUDED.notes,
        updated_at   = NOW();

    INSERT INTO device_sensors (device_id, sensor_def_id, gpio_pin, i2c_address, install_date, notes)
    VALUES (v_device_id, v_mq4_id, 'GPIO34', NULL, CURRENT_DATE, 'MQ4 on analog ADC1 channel 6 (GPIO34).')
    ON CONFLICT (device_id, sensor_def_id) DO UPDATE SET
        gpio_pin     = EXCLUDED.gpio_pin,
        notes        = EXCLUDED.notes,
        updated_at   = NOW();
    ...
    INSERT INTO device_sensors (device_id, sensor_def_id, gpio_pin, i2c_address, install_date, notes)
    VALUES (v_device_id, v_mq7_id, 'GPIO35', NULL, CURRENT_DATE, 'MQ7 on analog ADC1 channel 7 (GPIO35).')
    ON CONFLICT (device_id, sensor_def_id) DO UPDATE SET
        gpio_pin     = EXCLUDED.gpio_pin,
        notes        = EXCLUDED.notes,
        updated_at   = NOW();
    ...
    INSERT INTO device_sensors (device_id, sensor_def_id, gpio_pin, i2c_address, install_date, notes)
    VALUES (v_device_id, v_mq135_id, 'GPIO32', NULL, CURRENT_DATE, 'MQ135 on analog ADC1 channel 4 (GPIO32).')
    ON CONFLICT (device_id, sensor_def_id) DO UPDATE SET
        gpio_pin     = EXCLUDED.gpio_pin,
        notes        = EXCLUDED.notes,
        updated_at   = NOW();
    ...
    INSERT INTO device_sensors (device_id, sensor_def_id, gpio_pin, i2c_address, install_date, notes)
    VALUES (v_device_id, v_rtc_id, NULL, '0x68', CURRENT_DATE, 'DS3231 RTC on I2C bus, default address 0x68.')
    ON CONFLICT (device_id, sensor_def_id) DO UPDATE SET
    ...
    -- ----- Device Config -----
    INSERT INTO device_configs (device_id, config_key, config_value, description)
    VALUES (v_device_id, 'reading_interval_s', '30', 'Sensor reading interval in seconds.')
    ...
    INSERT INTO device_configs (device_id, config_key, config_value, description)
    VALUES (v_device_id, 'wifi_ssid', 'SIMOSI_AP', 'Wi-Fi network SSID for the device.')
    ON CONFLICT (device_id, config_key) DO UPDATE SET
        config_value = EXCLUDED.config_value,
        description  = EXCLUDED.description,
        updated_at   = NOW();
    ...
    INSERT INTO device_configs (device_id, config_key, config_value, description)
    VALUES (v_device_id, 'api_endpoint', 'http://api.simosi.local/api/v1/readings', 'FastAPI endpoint URL for posting readings.')
    ON CONFLICT (device_id, config_key) DO UPDATE SET
        config_value = EXCLUDED.config_value,
        description  = EXCLUDED.description,
        updated_at   = NOW();
    ...
    INSERT INTO device_configs (device_id, config_key, config_value, description)
    VALUES (v_device_id, 'deep_sleep_enabled', 'false', 'Enable deep sleep between readings to conserve battery.')
    ON CONFLICT (device_id, config_key) DO UPDATE SET
        config_value = EXCLUDED.config_value,
        description  = EXCLUDED.description,
        updated_at   = NOW();
    ...
    INSERT INTO device_states (device_id, is_online, power_state, last_seen_at)
    VALUES (v_device_id, FALSE, 'off', NULL)
    ...
    VALUES (
        ...
        1.200000,
        'Factory',
        'Default factory calibration values for MQ135 CO2 estimation. R0 measured in clean air at 20°C, 65% RH.'
    );

    RAISE NOTICE 'SIMOSI seed data inserted successfully. Device ID: %', v_device_id;

END $$;

-- ----- Sensor Thresholds -----
-- Source references:
-- PM2.5/PM10  : PP 22/2021 (Indonesia), WHO AQG 2021
-- CH4         : NIOSH REL, mining safety standards
-- H2S         : NIOSH REL, OSHA PEL
-- CO          : NIOSH REL (35 ppm TWA), OSHA PEL (50 ppm)
-- CO2         : ASHRAE, NIOSH REL (5000 ppm TWA)
...
VALUES
    (
        ...
        'PM2.5 thresholds aligned with Indonesian air quality standards and WHO guidelines.'
    ),
    (
    ...
    (
        ...
        'NIOSH REL & Mining Safety',
        'CH4 explosive range: 5-15% (50000-150000 ppm). Warning starts well below LEL.'
    ),
    (
        ...
        20.0001, 100.0000,
        'NIOSH REL & OSHA PEL',
        'H2S: NIOSH ceiling = 10 ppm (10 min). OSHA PEL = 20 ppm ceiling. IDLH = 50 ppm.'
    ),
    (
        ...
        200.0001, 1200.0000,
        'NIOSH REL & OSHA PEL',
        'CO: NIOSH TWA = 35 ppm, OSHA PEL = 50 ppm, IDLH = 1200 ppm.'
    ),
    (
        ...
        'ASHRAE & NIOSH REL',
        'CO2: NIOSH TWA = 5000 ppm, OSHA PEL = 5000 ppm, IDLH = 40000 ppm.'
    )
